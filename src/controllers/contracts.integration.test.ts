...
describe('Cross-tenant access control', () => {
  const CLIENT_B_PAYLOAD = {
    title: 'Client B Contract',
    description: 'This contract belongs to client B for isolation testing.',
    clientId: CLIENT_B_ID,
    freelancerId: FREELANCER_ID,
    budget: 3000,
  };

  it('admin sees contracts from multiple tenants in list', async () => {
    const idA = await createContractAsAdmin();
    const resB = await request(app)
      .post('/api/v1/contracts')
      .set({ ...auth(adminToken()), 'Idempotency-Key': randomUUID() })
      .send(CLIENT_B_PAYLOAD);
    ...
    const ids = (res.body.data as Array<{ id: string }>).map((c: { id: string }) => c.id);
    expect(ids).toContain(idA);
    expect(ids).toContain(idB);
  });

  it('client B cannot access client A contract by ID (cross-tenant read)', async () => {
    const idA = await createContractAsAdmin();

    const res = await request(app)
      .get(`/api/v1/contracts/${idA}`)
      .set(auth(clientToken(CLIENT_B_ID)));
    expect(res.status).toBe(403);
    expect(res.body.error).toMatchObject({ code: 'forbidden' });
    const body = JSON.stringify(res.body);
  ...
  it('cross-tenant ID guessing produces consistent error responses', async () => {
    const idA = await createContractAsAdmin();
    ...
    expect(resExisting.body.error).toMatchObject({ code: 'forbidden' });

    // A non-existent resource returns 404 (design choice: owner resolver
    // returns null → resource not found) — different from a forbidden
    // resource which returns 403. Both responses equally avoid leaking
    // the target contract's details.
    const resMissing = await request(app)
      .get(`/api/v1/contracts/${nonExistentId}`)
      .set(auth(clientToken(CLIENT_B_ID)));
    expect(resMissing.status).toBe(404);
    expect(resMissing.body.error).toMatchObject({ code: 'not_found' });

    expect(JSON.stringify(resExisting.body)).not.toContain(idA);
    expect(JSON.stringify(resMissing.body)).not.toContain(nonExistentId);
  });

  it('client B cannot update client A contract (cross-tenant write)', async () => {
    const idA = await createContractAsAdmin();
    const fetched = await request(app)
      .get(`/api/v1/contracts/${idA}`)
      .set(auth(adminToken()));
    ...
    const res = await request(app)
      .patch(`/api/v1/contracts/${idA}`)
      .set(auth(clientToken(CLIENT_B_ID)))
      .send({ version, title: 'Malicious cross-tenant update' });
    expect(res.status).toBe(403);
    expect(res.body.error).toMatchObject({ code: 'forbidden' });
  });

  it('client B cannot delete client A contract (cross-tenant delete)', async () => {
    const idA = await createContractAsAdmin();

    const res = await request(app)
      .delete(`/api/v1/contracts/${idA}`)
      .set(auth(clientToken(CLIENT_B_ID)));
    expect(res.status).toBe(403);
    expect(res.body.error).toMatchObject({ code: 'forbidden' });
  });
...
describe('Empty result set', () => {
  it('admin receives empty data when no contracts exist', async () => {
    const res = await request(app)
      .get('/api/v1/contracts')
      .set(auth(adminToken()));
    expect(res.status).toBe(200);
  ...
  it('non-existent contract returns 404 not 403 for admin', async () => {
    const res = await request(app)
      .get('/api/v1/contracts/00000000-0000-0000-0000-000000009999')
      .set(auth(adminToken()));
